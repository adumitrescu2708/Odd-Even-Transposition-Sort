...
void solve_oets() {
    ...
    for(int i = 0; i < N; i++) {
        ...
        if(rank == MASTER) {
            for(int j = 1; j < NUM_TASKS; j++) {
                int start = i % 2 == 0 ? start_even_idxs[j] : start_odd_idxs[j];
                MPI_Recv(v + start, end_idxs[j] != N ? end_idxs[j] - start + 1 : end_idxs[j] - start, MPI_INT, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
        }
    }  
}
